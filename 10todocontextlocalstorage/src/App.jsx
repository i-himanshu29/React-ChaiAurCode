...
function App() {
  // todos state to hold all the todos
  const [todos, setTodos] = useState([]); // this todo is an array

  const addTodo = (todo) => {
    // this todo is not from the state
    setTodos((prev) => [{ id: Date.now(), ...todo }, ...prev]); // add new todo at the top and spread the previous todos , id is unique
  };
  ...
  // Note : filter method creates a new array with all elements that pass the test implemented by the provided function.
  const deleteTodo = (id) => {
  ...
  const toggleComplete = (id) => {
    ...
    // jo id milegi uske completed ko toggle kar do
    setTodos((prev) =>
      prev.map((prevTodo) =>
        prevTodo.id === id
  ...
  // local storage me data key value pair me store hota hai
  ...
  // Here we want to get the values from the local storage
  useEffect(() => {
    const todos = JSON.parse(localStorage.getItem("todos")); // ye string lega isliye json parse kiya
    if (todos && todos.length > 0) { 
      setTodos(todos); 
    }
  }, []);
  // you can have multiple useEffect in one application
  ...
  // whenever todos change this useEffect will run
  useEffect(() => {
